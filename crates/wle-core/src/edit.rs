...
pub fn list_object_primitives_at(
    ...
    pointer: &str,
) -> Result<Vec<(String, JsonEditValue)>, String> {
    let node = value
        .pointer(pointer)
        ...
        let jv = match v {
            ...
            serde_json::Value::Number(n) => {
                if let Some(i) = n.as_i64() {
                    Some(JsonEditValue::Int(i))
                } else {
                    n.as_f64().map(JsonEditValue::Float)
                }
            }
...
pub fn apply_object_primitive_updates(
    value: &mut serde_json::Value,
    pointer: &str,
...
fn parent_pointer(ptr: &str) -> Option<(&str, &str)> {
    ...
    if let Some(pos) = ptr.rfind('/') {
        if pos == 0 {
            ...
            return Some((&ptr[..pos], &ptr[pos + 1..]));
        }
    }
    None
}

pub fn set_raw_by_pointer(
