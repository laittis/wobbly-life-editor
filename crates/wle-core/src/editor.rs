...
pub fn zip_backup_slot(dir: &Path) -> io::Result<PathBuf> {
    if !dir.is_dir() { return Err(io::Error::new(io::ErrorKind::InvalidInput, "not a directory")); }
    ...
    let options = FileOptions::default().compression_method(CompressionMethod::Deflated).unix_permissions(0o644);
    let base = dir;
...
fn write_lp_string(w: &mut Vec<u8>, s: &str) { write_7bit_len(w, s.len()); w.extend_from_slice(s.as_bytes()); }
fn write_7bit_len(w: &mut Vec<u8>, mut v: usize) { while v >= 0x80 { w.push(((v as u8) & 0x7F) | 0x80); v >>= 7; } w.push(v as u8); }
...
pub fn parse_guid_hyphen(s: &str) -> Result<Guid, String> {
    ...
    if parts.len() != 5 { return Err("invalid guid format".into()); }
    ...
    if parts[3].len() != 4 || parts[4].len() != 12 { return Err("bad d/e or tail".into()); }
    let b0 = u8::from_str_radix(&parts[3][0..2], 16).map_err(|_| "bad d0")?;
    let b1 = u8::from_str_radix(&parts[3][2..4], 16).map_err(|_| "bad d1")?;
    let mut tail = [0u8; 6];
    for (i, slot) in tail.iter_mut().enumerate() { let off = i * 2; *slot = u8::from_str_radix(&parts[4][off..off+2], 16).map_err(|_| "bad tail")?; }
    let a = p1 as i32; let b = (p2 as i16) as i32; let c = (p3 as i16) as i32; let d_to_k = [b0, b1, tail[0], tail[1], tail[2], tail[3], tail[4], tail[5]];
    Ok(Guid { a, b, c, d_to_k })
}

// Test helper: build a minimal SlotInfo.sav payload (BinaryFormatter)
pub fn build_slot_info_bytes(last_selected_player_slot: i32, date_time: &str, small_image_data: &[u8]) -> Vec<u8> {
    ...
    w.push(12);
    write_i32(&mut w, 2);
    write_lp_string(&mut w, "Game, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null");
