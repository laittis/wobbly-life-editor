...
impl Default for JsonOpts {
    fn default() -> Self { Self { max_array_elems: 128, max_depth: 16, bytes_summary: true } }
}
...
pub fn dump_file_json(path: &Path, opts: JsonOpts) -> Result<String, String> {
    ...
    match data.iter().copied().find(|b| !b.is_ascii_whitespace()) {
        Some(b'{') => std::str::from_utf8(&data).map(|s| s.to_string()).map_err(|_| "non-utf8 text".to_string()),
        Some(_) => {
            let leaked: &'static [u8] = Box::leak(data.into_boxed_slice());
            let mut parser = Parser::new(leaked);
...
pub fn dump_dynamic_json(doc: &Document<'_>, opts: JsonOpts) -> String {
    ...
    write!(&mut out, "{{\n  \"$rootClass\": \"{}\",\n  \"root\": ", doc.root_class_name().unwrap_or("<unknown>")).ok();
...
fn write_value_json(wctx: &Document<'_>, v: &Value<'_>, depth: usize, out: &mut String, opts: &JsonOpts) -> std::fmt::Result {
    match v {
        ...
        Value::Bytes(b) => if opts.bytes_summary {
        ...
        Value::Array(items) => {
            ...
            for (i, it) in items.iter().enumerate().take(max) {
                if i > 0 { out.push(','); }
                if depth >= opts.max_depth { out.push_str("null"); } else { write_value_json(wctx, it, depth + 1, out, opts)?; }
            }
            if items.len() > max { write!(out, ",{{\"$truncated\":true,\"$omitted\":{}}}", items.len() - max)?; }
            out.push(']');
        }
        Value::Object(obj) => {
            out.push('{');
            ...
            for (name, val) in obj.members.iter() {
                ...
                if depth >= opts.max_depth { out.push_str("null"); } else { write_value_json(wctx, val, depth + 1, out, opts)?; }
            }
            out.push('}');
        }
        Value::Ref(id) => {
...
fn escape_json(s: &str) -> String {
    ...
    for ch in s.chars() {
        match ch {
            ...
            '\r' => out.push_str("\\r"),
            '\t' => out.push_str("\\t"),
            c if c.is_control() => { use std::fmt::Write; write!(&mut out, "\\u{:04x}", c as u32).ok(); }
            c => out.push(c),
        }
    }
    out
...
pub fn find_sav_files(dir: &Path) -> Vec<PathBuf> {
    ...
    if let Ok(rd) = fs::read_dir(dir) {
        for entry in rd.flatten() {
            let p = entry.path();
            if p.is_file() && p.extension().and_then(|s| s.to_str()) == Some("sav") { out.push(p); }
        }
    }
    out.sort();
    out
...
pub fn dump_dir_map_json(dir: &Path, opts: JsonOpts) -> Result<String, String> {
    ...
    for (i, f) in files.iter().enumerate() {
        ...
        match dump_file_json(f, opts) { Ok(s) => out.push_str(&s), Err(e) => { write!(&mut out, "{{\"$error\":\"{}\"}}", escape_json(&e)).ok(); } }
    }
    out.push_str("\n}\n");
    Ok(out)
}
