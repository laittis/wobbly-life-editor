...
fn decode_slot_info_image_len() {
    ...
    let info = doc.as_save_slot_info().unwrap_or_else(|| panic!("slotinfo root={}", cls));
...
fn generic_json_pointer_edit_ops() {
    ...
    use wle_core::{parse_file_to_json_value, get_by_pointer, list_children, set_by_pointer, add_key, remove_at_pointer, array_insert, array_remove, write_json_to_file, JsonEditValue};
    ...
    assert_eq!(get_by_pointer(&v, "/root/a/c").unwrap(), serde_json::json!(false));
    // array insert/remove
    array_insert(&mut v, "/root/a/b", 1, serde_json::json!(42)).unwrap();
    assert_eq!(get_by_pointer(&v, "/root/a/b/1").unwrap(), serde_json::json!(42));
    array_remove(&mut v, "/root/a/b", 3).unwrap(); // remove former last element 3
    // add/remove key
    add_key(&mut v, "/root/a", "d", serde_json::json!("hi")).unwrap();
    assert_eq!(get_by_pointer(&v, "/root/a/d").unwrap(), serde_json::json!("hi"));
...
fn write_generic_binfmt_roundtrip() {
    use tempfile::tempdir;
    use wle_core::{write_binfmt_file_from_json, json};
    let dir = tempdir().unwrap();
